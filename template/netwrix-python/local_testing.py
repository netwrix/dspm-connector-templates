def validate_request_schema(config, request_data, function_type):
    ...
    if extra_properties:
        return False, f"Additional properties not allowed for {function_type}: {sorted(extra_properties)}"
...
def _validate_object_against_schema(data, schema_config, field_name):
    ...
    if extra_keys:
        return False, f"Additional properties not allowed in '{field_name}': {sorted(extra_keys)}"
    ...
    for field_config in schema_config:
        ...
        if field_type == "text" or field_type == "string":
            ...
            if isinstance(value, str):
                ...
                return (
                    False,
                    f"Field '{field_name}.{field_key}' must be a string or array, got {type(value).__name__}",
                )
            ...
            if options:
                ...
                for val in value_list:
                    if val not in valid_values:
                        return (
                            False,
                            f"Field '{field_name}.{field_key}' contains invalid value '{val}'. "
...
def _validate_location_against_columns(location_data, get_object_columns):
    ...
    if len(actual_columns) != len(expected_columns):
        return (
            False,
            f"Location must contain exactly {len(expected_columns)} columns. "
    ...
    for i, expected_col in enumerate(expected_columns):
        if i >= len(actual_columns) or actual_columns[i] != expected_col:
            return (
                False,
                f"Location columns must match order and names. Expected: {expected_columns}, Got: {actual_columns}",
            )
...
def validate_update_execution_params(
    status,
    total_objects,
    completed_objects,
    ...
    if total_objects is not None and not isinstance(total_objects, int) or total_objects < 0:
        return False, f"total_objects must be a non-negative integer, got: {total_objects}"

    # Validate completed_objects
    if completed_objects is not None and not isinstance(completed_objects, int) or completed_objects < 0:
    ...
    if (
        increment_completed_objects is not None
        and not isinstance(increment_completed_objects, int)
        or increment_completed_objects < 0
    ):
        return (
            False,
            f"increment_completed_objects must be a non-negative integer, got: {increment_completed_objects}",
        )
    ...
    if total_objects is not None and completed_objects is not None and completed_objects > total_objects:
        return (
            False,
            f"completed_objects ({completed_objects}) cannot be greater than total_objects ({total_objects})",
        )
    ...
    if completed_at is not None:
        ...
        if not re.match(iso8601_pattern, completed_at):
            return (
                False,
                f"completed_at must be in ISO8601 format (e.g., '2023-12-25T10:30:00Z'), got: '{completed_at}'",
            )
        ...
        try:
            ...
            for fmt in ["%Y-%m-%dT%H:%M:%SZ", "%Y-%m-%dT%H:%M:%S.%fZ", "%Y-%m-%dT%H:%M:%S%z", "%Y-%m-%dT%H:%M:%S.%f%z"]:
                try:
                    datetime.strptime(
                        completed_at.replace("Z", "+00:00") if completed_at.endswith("Z") else completed_at,
                        fmt.replace("%z", "+00:00") if fmt.endswith("%z") else fmt,
                    )
                    break
                except ValueError:
                    continue
...
def validate_get_object_response(response_data):
    ...
    if extra_keys:
        return False, f"Response body contains additional properties: {sorted(extra_keys)}"
...
def validate_test_connection_response(response_data):
    ...
    if missing_keys:
        return False, f"Response body missing required properties: {sorted(missing_keys)}"
    ...
    for field_name in ["startedAt", "completedAt"]:
        ...
        if not re.match(iso8601_pattern, field_value):
            return (
                False,
                f"Response body.{field_name} must be in ISO8601 format (e.g., '2023-12-25T10:30:00Z'), "
                f"got: '{field_value}'",
            )
        ...
        try:
            ...
            for fmt in ["%Y-%m-%dT%H:%M:%SZ", "%Y-%m-%dT%H:%M:%S.%fZ", "%Y-%m-%dT%H:%M:%S%z", "%Y-%m-%dT%H:%M:%S.%f%z"]:
                try:
                    datetime.strptime(
                        field_value.replace("Z", "+00:00") if field_value.endswith("Z") else field_value,
                        fmt.replace("%z", "+00:00") if fmt.endswith("%z") else fmt,
                    )
                    break
                except ValueError:
    ...
    try:
        ...
        if completed_dt < started_dt:
            return (
                False,
                f"Response body.completedAt ({body['completedAt']}) must be after startedAt ({body['startedAt']})",
            )
    except Exception:
    ...
    if expected_type == "String":
        ...
        if expected_type in ranges:
            ...
            if value < min_val or value > max_val:
                return (
                    False,
                    f"Column '{column_name}' value {value} is out of range for {expected_type} "
        ...
        if isinstance(value, str):
            ...
            return (
                False,
                f"Column '{column_name}' must be a datetime object or ISO8601 string, got {type(value).__name__}",
            )
        ...
        print(
            f"Warning: Unknown ClickHouse type '{expected_type}' for column '{column_name}', skipping type validation",
            flush=True,
        )
